  ...
  var rddArr = mutable.ArrayBuilder.make[RDD[(Int, Any)]]
	...
	var keyPosByInputName = mutable.Map[String, Int]()
        for (line <- Source.fromFile(inputPath).getLines()) {
          ...
          println("tablename: " + splittedLine(splittedLine.size - 1))
          if (splittedLine(splittedLine.size - 1) == "order_reviews.csv") {
            println("here")
            keyPosByInputName.put(line, 1)
          } else if (splittedLine(splittedLine.size - 1) == "ebay.csv") {
            keyPosByInputName.put(line, 2)
          } else {
