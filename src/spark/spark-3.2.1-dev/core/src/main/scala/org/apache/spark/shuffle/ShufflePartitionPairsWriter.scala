...
private[spark] class ShufflePartitionPairsWriter(
    ...
    checksum: Checksum)
  extends PairsWriter with Closeable with Serializable {
  ...
  override def write(key: Any, value: Any): Unit = {
    ...
    if (objOut == null) {
      ...
      open()
      //myLogger.warn("open(): " + (System.currentTimeMillis - ts))
    }
    val ts = System.currentTimeMillis
    objOut.writeKey(key)
    objOut.writeValue(value)
    recordWritten()
    //myLogger.warn("write(): " + (System.currentTimeMillis - ts))
  }
  ...
  private def open(): Unit = {
    try {
      ...
      assert(checksumOutputStream == null, "checksumOutputStream != null")
