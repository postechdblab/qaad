...
final class ShuffleBlockFetcherIterator(
  ...
  extends Iterator[(BlockId, InputStream)] with DownloadFileManager with Logging with Serializable {
  ...
  private[storage] def cleanup(): Unit = {
    ...
    while (iter.hasNext) {
      ...
      result match {
        case SuccessFetchResult(blockId, mapIndex, address, _, buf, _, _, _) =>
  ...
  private[this] def sendRequest(req: FetchRequest): Unit = {
    ...
    val infoMap = req.blocks.map {
      case FetchBlockInfo(blockId, size, mapIndex) => (blockId.toString, (size, mapIndex, blockId.asInstanceOf[ShuffleBlockId].remainder, blockId.asInstanceOf[ShuffleBlockId].mod))
    }.toMap
    ...
    @inline def enqueueDeferredFetchRequestIfNecessary(): Unit = {
      if (remainingBlocks.isEmpty && deferredBlocks.nonEmpty) {
        val blocks = deferredBlocks.map { blockId =>
          val (size, mapIndex, _, _) = infoMap(blockId)
    ...
    val blockFetchingListener = new BlockFetchingListener {
      override def onBlockFetchSuccess(blockId: String, buf: ManagedBuffer): Unit = {
        //println("onBlockFetchSuccess: " + blockId)
        ...
        ShuffleBlockFetcherIterator.this.synchronized {
          if (!isZombie) {
            ...
            results.put(new SuccessFetchResult(BlockId(blockId)/* blockIdToPut*/, infoMap(blockId)._2,
              address, infoMap(blockId)._1, buf, remainingBlocks.isEmpty, infoMap(blockId)._3, infoMap(blockId)._4))
  ...
  override def next(): (BlockId, InputStream) = {
    ...
    while (result == null) {
      ...
      //myLogger.warn("fetchWaitTime: " + fetchWaitTime)
      shuffleMetrics.incFetchWaitTime(fetchWaitTime)

      result match {
        case r @ SuccessFetchResult(blockId, mapIndex, address, size, buf, isNetworkReqDone, remainder, mod) =>
          //println("SuccessFetchResult blockId: " + blockId)
          blockId match {
            case b: ShuffleBlockId =>
              if (b.remainder == -1) {
          ...
          val in = try {
            ...
            if (checksumEnabled) {
              //assert(false, "checksumEnabled == true")
...
object ShuffleBlockFetcherIterator {
  ...
  def mergeContinuousShuffleBlockIdsIfNeeded(
    ...
    val result = if (doBatchFetch) {
      ...
      def mergeFetchBlockInfo(toBeMerged: ArrayBuffer[FetchBlockInfo]): FetchBlockInfo = {
        ...
        assert(startBlockId.remainder != -1, "startBlockId.remainder == -1")
      ...
      while (iter.hasNext) {
        ...
        if (info.blockId.isInstanceOf[ShuffleBlockBatchId]) {
          ...
          if (curBlocks.isEmpty) {
            ...
            assert(curBlockId.remainder != -1, "curBlock.remainder == -1")
  ...
  private[storage] case class SuccessFetchResult(
      ...
      remainder: Long = -1L,
      mod: Long = -1L) extends FetchResult {
    require(buf != null)
    require(size >= 0)
  }
