...
class SparkContext(config: SparkConf) extends Logging with Serializable {
  ...
  def union[T: ClassTag](rdds: Seq[RDD[T]]): RDD[T] = withScope {
    ...
    //println("partitioner.size: " + partitioners.size)
    //println("rdds: " + rdds.map(rdd => rdd.id).toList)
    for (rdd <- nonEmptyRdds) {
      //println("rdd: " + rdd.collect().toList)
    }
    //println("# of non empty rdds: " + nonEmptyRdds.size)
    if (nonEmptyRdds.forall(_.partitioner.isDefined) && partitioners.size == 1) {
      //println("PartitionerAwareUnionRDD")
  ...
  def runJob[T, U: ClassTag](
    ...
    //println("# of partitions in runJob: " + partitions.size)
  ...
  def runJob[T, U: ClassTag](
    ...
    // val results = new Array[U](partitions.size) // (modification)
    var results = scala.collection.mutable.Map[Int, U]() // (modification)
    runJob[T, U](rdd, func, partitions, (index, res) => results(index) = res)
    results.values.toArray // (modification)
  }
