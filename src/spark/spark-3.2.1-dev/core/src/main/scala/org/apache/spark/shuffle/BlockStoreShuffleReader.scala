...
private[spark] class BlockStoreShuffleReader[K, C](
  ...
  extends ShuffleReader[K, C] with Logging with Serializable {
  ...
  override def read(): Iterator[Product2[K, C]] = {
    ...
    val recordIter = wrappedStreams.flatMap { case (blockId, wrappedStream) =>
      //println("blockId in recordIter: " + blockId)
      var deserializedIter = serializerInstance.deserializeStream(wrappedStream).asKeyValueIterator
      blockId match {
        case b: org.apache.spark.storage.ShuffleBlockId =>
          //assert(b.remainder != -1)
          deserializedIter = deserializedIter.filter(x => x._1.asInstanceOf[(Int, _)]._2.hashCode % b.mod.toInt == b.remainder.toInt)
          //  .duplicate
          //println("deserializedIter: " + tmp._2.toList)
          //deserializedIter = tmp._1
        case _ =>
      }
