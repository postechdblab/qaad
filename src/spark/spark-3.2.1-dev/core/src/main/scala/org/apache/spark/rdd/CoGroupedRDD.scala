...
class CoGroupedRDD[K: ClassTag](
  ...
  override def getPartitions: Array[Partition] = {
    ...
    val array = if (this.updatedPartitioner == null) new Array[Partition](part.numPartitions) else new Array[Partition](this.updatedPartitioner.asInstanceOf[org.apache.spark.HashPartitioner].partitions)
  ...
  override def compute(s: Partition, context: TaskContext): Iterator[(K, Array[Iterable[_]])] = {
    ...
    for ((dep, depNum) <- dependencies.zipWithIndex) dep match {
      case oneToOneDependency: OneToOneDependency[Product2[K, Any]] @unchecked =>
        //println("oneToOneDependency in CoGroupedRDD")
      ...
      case shuffleDependency: ShuffleDependency[_, _, _] =>
        //println("shuffleDependency in CoGroupedRDD")
        ...
        val it = SparkEnv.get.shuffleManager
          ...
          .read()
        //println("read in CoGroupedRDD: " + ((System.currentTimeMillis - startTime3) / 1000.0f))
        //try {
          //println(s"it(${split.index}): " + tmp._2.toList)
        //} catch {
          //println("split.index: " + split.index)
        //}
        rddIterators += ((it, depNum))
    }
    ...
    //println("createExternalMap in CoGroupedRDD: " + ((System.currentTimeMillis - startTime2) / 1000.0f))
