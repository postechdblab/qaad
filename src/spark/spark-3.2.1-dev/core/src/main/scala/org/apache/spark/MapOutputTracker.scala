...
private class ShuffleStatus(
    ...
    var numReducers: Int = -1) extends Logging {
   ...
   */
  // Exposed for testing
  var mapStatuses = new Array[MapStatus](numPartitions)

  /**
   * Keep the previous deleted MapStatus for recovery.
   */
  var mapStatusesDeleted = new Array[MapStatus](numPartitions)
  ...
  private[this] var cachedSerializedBlocksByReduceId: Array[Byte] = _
  ...
  private[spark] var cachedSerializedBroadcastBlocksByReduceId: Broadcast[Array[Array[Byte]]] = _
  ...
  def addMapOutput(mapIndex: Int, status: MapStatus): Unit = withWriteLock {
    if (mapIndex >= mapStatuses.size) {
      mapStatuses = mapStatuses ++ Array.fill[MapStatus](mapIndex + 1 - numPartitions)(null)
      numPartitions = mapIndex + 1
    }
   ...
   */
  def findMissingPartitions(_numPartitions: Int): Seq[Int] = withReadLock {
    val numPartitions = if (_numPartitions == -1) this.numPartitions else _numPartitions 
  ...
  def serializedBlocksByReduceId(
      broadcastManager: BroadcastManager,
      isLocal: Boolean,
      minBroadcastSize: Int,
      conf: SparkConf): Array[Byte] = {
    ...
    var result: Array[Byte] = null
    withReadLock {
      if (cachedSerializedBlocksByReduceId != null) {
        result = cachedSerializedBlocksByReduceId
      }
    }

    if (result == null) withWriteLock {
      if (cachedSerializedBlocksByReduceId == null) {
        val serResult = MapOutputTracker.serializeBlocksByReduceId(
          broadcastManager, isLocal, minBroadcastSize, conf)
        cachedSerializedBlocksByReduceId = serResult._1
        cachedSerializedBroadcastBlocksByReduceId = serResult._2
      }
      result = cachedSerializedBlocksByReduceId
    }
    result
...
private[spark] case class GetMapAndMergeResultStatuses(shuffleId: Int)
  extends MapOutputTrackerMessage
private[spark] case class GetBlocksByReduceId(shuffleId: Int)
  extends MapOutputTrackerMessage
...
private[spark] case class GetMapAndMergeOutputMessage(shuffleId: Int,
  context: RpcCallContext) extends MapOutputTrackerMasterMessage
private[spark] case class GetBlocksByReduceIdMessage(shuffleId: Int,
  context: RpcCallContext) extends MapOutputTrackerMasterMessage
...
private[spark] class MapOutputTrackerMasterEndpoint(
  ...
  override def receiveAndReply(context: RpcCallContext): PartialFunction[Any, Unit] = {
    ...
    case GetBlocksByReduceId(shuffleId: Int) =>
      val hostPort = context.senderAddress.hostPort
      logInfo(s"Asked to send map output locations for shuffle $shuffleId to $hostPort")
      tracker.post(GetBlocksByReduceIdMessage(shuffleId, context))
...
private[spark] class MapOutputTrackerMaster(
  ...
  private class MessageLoop extends Runnable {
    private def handleStatusMessage(
        ...
        needBlocksByReduceId: Boolean = false): Unit = {
      val hostPort = context.senderAddress.hostPort
      ...
      if (needBlocksByReduceId) {
        context.reply(
          shuffleStatus.serializedBlocksByReduceId(broadcastManager, isLocal, minSizeForBroadcast, conf))
      } else if (needMergeOutput) {
        context.reply(
          shuffleStatus.
        ...
        while (true) {
          try {
            ...
            data match {
              ...
              case GetBlocksByReduceIdMessage(shuffleId, context) =>
                handleStatusMessage(shuffleId, context, false, true)
            }
          } catch {
  ...
  def registerShuffle(shuffleId: Int, numMaps: Int, numReduces: Int): Unit = {
    if (pushBasedShuffleEnabled) {
      ...
      val numPartitions = if (MapOutputTracker.blocksByReduceId.contains(shuffleId)) {
        MapOutputTracker.blocksByReduceId(shuffleId).size
      } else {
        numMaps
      }
      if (shuffleStatuses.put(shuffleId, new ShuffleStatus(numPartitions/*numMaps*/)).isDefined) {
        throw new IllegalArgumentException("Shuffle ID " + shuffleId + " registered twice")
      }
    }
  }
  ...
  def findMissingPartitions(shuffleId: Int): Option[Seq[Int]] = {
    shuffleStatuses.get(shuffleId).map(_.findMissingPartitions(if (MapOutputTracker.blocksByReduceId.contains(shuffleId)) MapOutputTracker.blocksByReduceId(shuffleId).size else -1))
  }
  ...
  def getStatisticsOfMapTasks(dep: ShuffleDependency[_, _, _]): MapOutputStatistics = {
    shuffleStatuses(dep.shuffleId).withMapStatuses { statuses =>
      val totalSizes = new Array[Long](dep._rdd.getNumPartitions)
      val m = dep.partitioner.numPartitions
      //val totalSizes = new Array[Long](dep.partitioner.numPartitions)
      val parallelAggThreshold = conf.get(
        SHUFFLE_MAP_OUTPUT_PARALLEL_AGGREGATION_THRESHOLD)
      val parallelism = math.min(
        Runtime.getRuntime.availableProcessors(),
        statuses.length.toLong * m / parallelAggThreshold + 1).toInt
      if (parallelism <= 1) {
        var cnt = -1
        for (s <- statuses) {
          cnt = cnt + 1
          for (i <- 0 until m) {
            totalSizes(cnt) += s.getSizeForBlock(i)
          }
        }
      } else {
        val threadPool = ThreadUtils.newDaemonFixedThreadPool(parallelism, "map-output-aggregate")
  ...
  def getStatisticsOfMicroRdds(dep: ShuffleDependency[_, _, _]): scala.collection.mutable.Map[Int, scala.collection.mutable.Map[Long, Long]] = {
    import org.apache.spark.scheduler.CompressedMapStatus
    shuffleStatuses(dep.shuffleId).withMapStatuses { statuses =>
      var totalSizes = scala.collection.mutable.Map[Int, scala.collection.mutable.Map[Long, Long]]().withDefaultValue(scala.collection.mutable.Map[Long, Long]())
      val parallelAggThreshold = conf.get(
        SHUFFLE_MAP_OUTPUT_PARALLEL_AGGREGATION_THRESHOLD)
      val parallelism = math.min(
        Runtime.getRuntime.availableProcessors(),
        statuses.length.toLong * dep._rdd.getNumPartitions / parallelAggThreshold + 1).toInt
      if (parallelism <= 1) {
        for (s <- statuses) {
          s match {
            case s: CompressedMapStatus =>
              assert(s.compressedSizeByMicroRddId != null, "compressedSizeByMicroRddId == null")
							for (kv <- s.compressedSizeByMicroRddId) {
								if (totalSizes.contains(kv._1)) {
                  if (totalSizes(kv._1).contains(s.mapId)) {
                    totalSizes(kv._1)(s.mapId) += s.getSizeForBlock(kv._1)
                  } else {
                    totalSizes(kv._1).put(s.mapId, s.getSizeForBlock(kv._1))
                  }
								} else {
									totalSizes.put(kv._1, scala.collection.mutable.Map[Long, Long](s.mapId -> s.getSizeForBlock(kv._1)))
								}
							}
            case _ =>
          }
        ...
        try {
          implicit val executionContext = ExecutionContext.fromExecutor(threadPool)
          val mapStatusSubmitTasks = equallyDivide(dep._rdd.getNumPartitions, parallelism).map {
            reduceIds => Future {
              for (s <- statuses) {
								s match {
									case s: CompressedMapStatus =>
    ...
    if (shuffleStatus != null) {
      ...
      val preferredLoc = if (pushBasedShuffleEnabled) {
        shuffleStatus.withMergeStatuses { statuses =>
          if (statuses.size <= partitionId) { // (modification)
            flag = true
            Nil
          } else {
      ...
      if (dep.rdd.updatedPartitioner != null) { // Why cannot this condition hold?
        Nil
      } else {
...
private[spark] class MapOutputTrackerWorker(conf: SparkConf) extends MapOutputTracker(conf) {
  ...
  override def getMapSizesByExecutorId(
      ...
      startPartition: Int,
      endPartition: Int): Iterator[(BlockManagerId, Seq[(BlockId, Long, Int)])] = {
    updateBlocksByReduceId(shuffleId, conf)
  ...
  private def getStatuses(
    ...
    if (canFetchMergeResult) {
      assert(false, "canFetchMergeResult")
      ...
      if (mapOutputStatuses == null || mergeOutputStatuses == null) {
        ...
        fetchingLock.withLock(shuffleId) {
          ...
          if (fetchedMapStatuses == null || fetchedMergeStatuses == null) {
            ...
            //println("fetchedMapStatuses: " + fetchedMapStatuses.map { status =>
            //  status.mapId
            //}.toList + ", shuffleId: " + shuffleId)
      ...
      if (statuses == null) {
        ...
        fetchingLock.withLock(shuffleId) {
          ...
          if (fetchedStatuses == null) {
            ...
            //println("fetchedStatuses: " + fetchedStatuses.map { status =>
            //  status.mapId
            //}.toList + ", shuffleId: " + shuffleId)
          }
  ...
  private def updateBlocksByReduceId(
      shuffleId: Int,
      conf: SparkConf): Unit = {
		if (!MapOutputTracker.blocksByReduceId.contains(shuffleId)) {
			fetchingLock.withLock(shuffleId) {
				val fetchedBytes = askTracker[Array[Byte]](GetBlocksByReduceId(shuffleId))
				try {
					val fetchedBlocksByReduceId =
						MapOutputTracker.deserializeBlocksByReduceId(fetchedBytes, conf).toMap
					MapOutputTracker.blocksByReduceId ++= fetchedBlocksByReduceId
				} catch {
					case e: SparkException =>
						throw new MetadataFetchFailedException(shuffleId, -1,
							s"Unable to deserialize blocksByReduceId for shuffle $shuffleId: " +
								e.getCause)
				}
			}
		}
  ...
  def unregisterShuffle(shuffleId: Int): Unit = {
    ...
    AdaptivePartitioner.idxByMicroRddId.remove(shuffleId)
    MapOutputTracker.blocksByReduceId.remove(shuffleId)
  }
...
private[spark] object MapOutputTracker extends Logging with Serializable {
  //@transient lazy val myLogger: org.apache.log4j.Logger = org.apache.log4j.LogManager.getLogger("myLogger")

  type BlocksByReduceId = Array[(Int, scala.collection.mutable.Map[Int, Array[(ShuffleBlockId, Long, Long)]])]
  ...
  var idxByMicroRddId = scala.collection.mutable.Map[(Int, Long, Int), Int]()
  var blocksByReduceId = scala.collection.mutable.Map[Int, scala.collection.mutable.Map[Int, Array[(ShuffleBlockId, Long, Long)]]]()
  ...
  def serializeBlocksByReduceId(
    ...
    val objOut = new ObjectOutputStream(codec.compressedOutputStream(out))
    Utils.tryWithSafeFinally {
      blocksByReduceId.synchronized {
        objOut.writeObject(MapOutputTracker.blocksByReduceId.toArray)
      }
    } {
      objOut.close()
    }
  ...
  def deserializeBlocksByReduceId(
      bytes: Array[Byte], conf: SparkConf): BlocksByReduceId = {
    assert (bytes.length > 0)
    ...
    bytes(0) match {
      case DIRECT =>
        deserializeObject(in).asInstanceOf[BlocksByReduceId]
      case BROADCAST =>
        try {
          // deserialize the Broadcast, pull .value array out of it, and then deserialize that
          val bcast = deserializeObject(in).asInstanceOf[Broadcast[Array[Array[Byte]]]]
          ...
          // Important - ignore the DIRECT tag ! Start from offset 1
          bcastIn.skip(1)
          deserializeObject(bcastIn).asInstanceOf[BlocksByReduceId]
        } catch {
          case e: IOException =>
            logWarning("Exception encountered during deserializing broadcasted" +
              " BlocksByReduceId: ", e)
  ...
  def convertMapStatuses(
    ...
    if (mergeStatuses.exists(_.exists(_ != null)) && startMapIndex == 0
      ...
      assert(false, "startMapIndex == 0 and endMapIndex == mapStatuses.length")
        ...
        case (mergeStatus, partId) =>
          val remainingMapStatuses = if (mergeStatus != null && mergeStatus.totalSize > 0) {
            ...
            splitsByAddress.getOrElseUpdate(mergeStatus.location, ListBuffer()) +=
              ((ShuffleMergedBlockId(shuffleId, mergeStatus.shuffleMergeId, partId), // (modification)
...
/*
        ...
        val iter = mapStatuses.iterator.zipWithIndex
				for ((status, mapIndex) <- iter.slice(startMapIndex, endMapIndex)) {
					...
					for (part <- startPartition until endPartition) {
						...
						if (size != 0) {
							var blockIdToPut = ShuffleBlockId(shuffleId, status.mapId, part)
							blockIdToPut.remainder = 0
							blockIdToPut.mod = 1
							splitsByAddress.getOrElseUpdate(status.location, ListBuffer()) +=
								((blockIdToPut, size, mapIndex))
						}
					}
				}
      } else {*/
				val blocksByReduceId = MapOutputTracker.blocksByReduceId(shuffleId)
				val mapStatusesIterator = mapStatuses.toList.filter(x => x != null)
				val statusMap = mapStatusesIterator.map(x => (x.mapId, x)).toMap
				val statusIndex = mapStatusesIterator.zipWithIndex.map { case (status, idx) =>
					(status.mapId, idx)
				}.toMap
				//println("shuffleId: " + shuffleId)
				//println("statusMap.keys: " + statusMap.keys.toList)
				//println("startPartition: " + startPartition)
				//println("endPartition: " + endPartition)
				//println("blocksByReduceId: " + blocksByReduceId.map(x => (x._1, x._2.toList)))
				for (reduceId <- startPartition until endPartition) {
					assert(blocksByReduceId.contains(reduceId))
					for ((blockId, remainder, mod) <- blocksByReduceId(reduceId)) {
						val shuffleId = blockId.shuffleId
						val mapId = blockId.mapId
						val microRddId = blockId.reduceId
            if (statusMap.contains(mapId)) {
							val status = statusMap(mapId)
							val size = status.getSizeForBlock(microRddId)
							if (size != 0) {
								var blockIdToPut = ShuffleBlockId(shuffleId, mapId, microRddId)
								blockIdToPut.remainder = remainder
								blockIdToPut.mod = mod
								assert(blockIdToPut.remainder != -1, "remainder == -1")
								splitsByAddress.getOrElseUpdate(status.location, ListBuffer()) +=
									((blockIdToPut, size, statusIndex(mapId)))
							}
            }
					}
				}
    ...
    //println("splitsByAddress: " + splitsByAddress.mapValues(_.toList).toList)
