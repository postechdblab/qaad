...
private[spark] class ExternalSorter[K, V, C](
  ...
  def insertAll(records: Iterator[Product2[K, V]]): Unit = {
    ...
    if (shouldCombine) {
      ...
      while (records.hasNext) {
        addElementsRead()
        kv = records.next()
        map.changeValue((/*getPartition(*/kv._1.asInstanceOf[(Int, _)]._1/*)*/, kv._1), update)
      ...
      while (records.hasNext) {
        ...
        val kv = records.next()
        //println("kv: " + kv)
        buffer.insert(/*getPartition(*/kv._1.asInstanceOf[(Int, _)]._1/*)*/, kv._1, kv._2.asInstanceOf[C])
  ...
  def writePartitionedFile(
    ...
    assert(false, "writePartitionedFile")
      ...
      while (it.hasNext) {
        ...
        TryUtils.tryWithSafeFinally {
          ...
          partitionPairsWriter = new ShufflePartitionPairsWriter(
            ...
            blockId,
            context.taskMetrics().shuffleWriteMetrics,
            /*if (partitionChecksums.nonEmpty) partitionChecksums(partitionId) else */null) //(tmp)
          //println("write block: " + blockId)
          while (it.hasNext && it.nextPartition() == partitionId) {
