...
import org.apache.spark.AdaptivePartitioner
...
private[spark] object MapStatus {
  ...
  def apply(
      ...
      mapTaskId: Long,
      uncompressedSizesByMicroRddId: scala.collection.mutable.Map[Int, Long] = null,
      uncompressedSizesByMicroRddIdAsJava: java.util.Map[java.lang.Integer, java.lang.Long] = null,
      shuffleId: Int = -1): MapStatus = {
    if (uncompressedSizes.length > minPartitionsToUseHighlyCompressMapStatus) {
      HighlyCompressedMapStatus(loc, uncompressedSizes, mapTaskId, uncompressedSizesByMicroRddId, uncompressedSizesByMicroRddIdAsJava)
    } else {
      assert(uncompressedSizesByMicroRddId != null, "uncompressedSizesByMicroRddId == null")
      assert(shuffleId != -1)
      new CompressedMapStatus(loc,
															uncompressedSizes.map(_.toByte),
															mapTaskId,
															uncompressedSizesByMicroRddId.map(x => (x._1, MapStatus.compressSize(x._2))),
															null,
															shuffleId)
    }
  }
...
private[spark] class CompressedMapStatus(
    ...
    /*private[this]*/ var compressedSizeByMicroRddId: scala.collection.mutable.Map[Int, Byte] = null,
    private[this] var compressedSizeByMicroRddIdAsJava: java.util.Map[java.lang.Integer, java.lang.Long] = null,
    private[this] val shuffleId: Int = -1)
  extends MapStatus with Externalizable {

  // For deserialization only
  protected def this() = this(null, null.asInstanceOf[Array[Byte]], -1, null.asInstanceOf[scala.collection.mutable.Map[Int, Byte]])
  ...
  def this(loc: BlockManagerId,
           uncompressedSizes: Array[Long],
           mapTaskId: Long,
           uncompressedSizesByMicroRddId: scala.collection.mutable.Map[Int, Long]) = {
    this(loc,
         uncompressedSizes.map(MapStatus.compressSize),
         mapTaskId,
         if (uncompressedSizesByMicroRddId != null) {
           uncompressedSizesByMicroRddId.map(x => (x._1, MapStatus.compressSize(x._2)))
         } else {
           null
         })
  ...
  def idxByMicroRddId = {
    val partitionLengths =
      compressedSizeByMicroRddId.map(x => (x._1, MapStatus.decompressSize(x._2))).toList.sortBy(_._1).zipWithIndex
    partitionLengths.map(x => ((shuffleId, _mapTaskId, x._1._1), x._2))
  }
  ...
  override def getSizeForBlock(reduceId: Int): Long = {
    if (compressedSizeByMicroRddId != null && compressedSizeByMicroRddId.contains(reduceId)) {
      MapStatus.decompressSize(compressedSizeByMicroRddId(reduceId))
    } else if (compressedSizeByMicroRddId != null && !compressedSizeByMicroRddId.contains(reduceId)) {
      0L
    } else {
      assert(false, "compressedSizes does not contain the size for a given reduceId")
  ...
  override def writeExternal(out: ObjectOutput): Unit = Utils.tryOrIOException {
    ...
    out.writeInt(compressedSizeByMicroRddId.size)
    for ((microRddId, size) <- compressedSizeByMicroRddId) {
      out.writeInt(microRddId)
      out.writeByte(size)
    }

    val indexes = idxByMicroRddId
    out.writeInt(indexes.size)
    for (((shuffleId, mapId, microRddId), idx) <- indexes) {
  ...
  override def readExternal(in: ObjectInput): Unit = Utils.tryOrIOException {
    ...
    val numKeys = in.readInt()
    compressedSizeByMicroRddId = scala.collection.mutable.Map[Int, Byte]()
    for (i <- 0 until numKeys) {
      val microRddId = in.readInt()
      val size = in.readByte()
      compressedSizeByMicroRddId.put(microRddId, size)
    }

    val numIndexes = in.readInt()
    for (i <- 0 until numIndexes) {
      val shuffleId = in.readInt()
      val mapId = in.readLong()
      val microRddId = in.readInt()
      val idx = in.readInt()
      AdaptivePartitioner.idxByMicroRddId.getOrElseUpdate(shuffleId, mutable.Map[(Long, Int), Int]())
        .put((mapId, microRddId), idx)
    }
  }
}
...
private[spark] object HighlyCompressedMapStatus {
  def apply(
      ...
      mapTaskId: Long,
      uncompressedSizesByMicroRddId: scala.collection.mutable.Map[Int, Long] = null,
      uncompressedSizesByMicroRddIdAsJava: java.util.Map[java.lang.Integer, java.lang.Long] = null): HighlyCompressedMapStatus = {
