...
private[spark] class DAGScheduler(
  ...
  private var globalWaiter: JobWaiter[_] = null // (modification)
  ...
  def getCacheLocs(rdd: RDD[_]): IndexedSeq[Seq[TaskLocation]] = cacheLocs.synchronized {
    ...
    if (cacheLocs(rdd.id).size != rdd.partitions.length) {
  ...
  type SizesOfMicroRdds = mutable.Map[Int, mutable.Map[Long, Long]]

  private def getDesiredPartitionSize(sizesOfMicroRdds: mutable.Map[Int, mutable.Map[Long, (Int, Long)]],
                                      minPartitionSize: Long = 100000000L, // -1L,
                                      maxPartitionSize: Long = 200000000L, // 999999999L,
                                      numExecutorCores: Int = 56): Long = {
    val sumOfEachRdd = sizesOfMicroRdds.map(kv => kv._2.values.map(_._2).sum)
    val maxSize = if (sizesOfMicroRdds.size == 0) 0 else sumOfEachRdd.max
    scala.math.min(scala.math.min(scala.math.max(minPartitionSize, maxSize),
                   maxPartitionSize),
                   sumOfEachRdd.sum / (numExecutorCores))
    //val tmp = maxSize
    //println("getDesiredPartitionSize: " + tmp)
    //tmp
  }

  private def sortSizesOfMicroRdds(sizesOfMicroRdds: mutable.Map[Int, mutable.Map[Long, (Int, Long)]]): Array[(Int, Array[(Long, (Int, Long))])] = {
    sizesOfMicroRdds.map { case (microRddId: Int, sizes: mutable.Map[Long, (Int, Long)])  =>
      (/*sizes.values.map(_._2).sum*/microRddId, microRddId, sizes.toArray)
    }.toArray.sortBy(-_._1).map { case (totalSize, microRddId, sizes) =>
      (microRddId, sizes)
    }
  }

  private def getNumSplittedPartitions(size: Long, desiredPartitionSize: Long): Long = {
    if (size % desiredPartitionSize == 0) {
      size / desiredPartitionSize
    } else {
      size / desiredPartitionSize + 1
    }
  }

  private def assignBlocksToReduceId(shuffleIds: Array[Int],
                                      sizesOfMicroRdds: mutable.Map[Int, mutable.Map[Long, (Int, Long)]])
                                      : mutable.Map[Int, mutable.Map[Int, Array[(ShuffleBlockId, Long, Long)]]] = {
    var startTime = System.currentTimeMillis
    val desiredPartitionSize = getDesiredPartitionSize(sizesOfMicroRdds)
    //println("desiredPartitionSize: " + desiredPartitionSize)
    val sortedSizesOfMicroRdds = sortSizesOfMicroRdds(sizesOfMicroRdds)
    var blocksByReduceId = mutable.Map[Int, mutable.Map[Int, Array[(ShuffleBlockId, Long, Long)]]]()
    var curReduceId: Int = 0
    var curBlocks = scala.collection.mutable.ArrayBuilder.make[ShuffleBlockId]
    var curSize: Long = 0L
    shuffleIds.foreach { shuffleId =>
      blocksByReduceId.put(shuffleId, mutable.Map[Int, Array[(ShuffleBlockId, Long, Long)]]())
    }
    sortedSizesOfMicroRdds.foreach { case (microRddId, sizes) =>
      val size = sizes.map(_._2).map(_._2).sum
      //println("microRddId: " + microRddId + ", size: ToReduceId(shuffleIds.result().toArray, " + size + ", desiredPSize: " + desiredPartitionSize)
			val targetBlocks = sizes.map(x => ShuffleBlockId(x._2._1, x._1, microRddId))
			if (curSize + size <= desiredPartitionSize) {
				curBlocks ++= targetBlocks
				curSize += size
			} else {
				if (curBlocks.result().size > 0) {
          shuffleIds.foreach { shuffleId =>
            val prev = blocksByReduceId.getOrElse(shuffleId, mutable.Map[Int, Array[(ShuffleBlockId, Long, Long)]]())
            blocksByReduceId.put(shuffleId, prev ++ mutable.Map(curReduceId -> curBlocks.result().map(x => (x, 0L, 1L)).filter(x => x._1.shuffleId == shuffleId)))
          }
					curReduceId += 1
					curBlocks = scala.collection.mutable.ArrayBuilder.make[ShuffleBlockId]
					curSize = 0
				}
				if (size > desiredPartitionSize) {
					val numSplittedPartitions = getNumSplittedPartitions(size, desiredPartitionSize)
					(0L until numSplittedPartitions).foreach { i =>
						shuffleIds.foreach { shuffleId =>
							val prev = blocksByReduceId.getOrElse(shuffleId, mutable.Map[Int, Array[(ShuffleBlockId, Long, Long)]]())
						  blocksByReduceId.put(shuffleId, prev ++ mutable.Map(curReduceId -> targetBlocks.map(blockId => (blockId, i, numSplittedPartitions)).filter(x => x._1.shuffleId == shuffleId)))
						}
						curReduceId += 1
					}
				} else {
    ...
    //println("# of reduce tasks: " + curReduceId)
    blocksByReduceId
  }

  private def getMissingParentStages(stage: Stage): List[Stage] = {
    //println("getMissingParentStages()")
    var startTime = System.currentTimeMillis
    ...
    // caused by recursively visiting
    val waitingForVisit = new ListBuffer[RDD[_]]
    val rddToUpdate = new ListBuffer[RDD[_]]
    waitingForVisit += stage.rdd
    var updatedPartitioner: Partitioner = null
    val tracker = SparkEnv.get.mapOutputTracker.asInstanceOf[org.apache.spark.MapOutputTrackerMaster]
    def visit(rdd: RDD[_]): Unit = {
      if (!visited(rdd)) {
        visited += rdd
        val rddHasUncachedPartitions = getCacheLocs(rdd).contains(Nil)
        if (rddHasUncachedPartitions/* && rdd.updatedPartitioner == null*/) {
          var shuffleIds = scala.collection.mutable.ArrayBuilder.make[Int]
          var sizesByMicroRddId = mutable.Map[Int, mutable.Map[Long, (Int, Long)]]()
          val prev = rddToUpdate.size
          var numShufDeps = 0
            ...
            dep match {
              case shufDep: ShuffleDependency[_, _, _] =>
                ...
                if (mapStage.isAvailable && mapStage.shuffleDep.shuffleMergeFinalized) {
                  shuffleIds += shufDep.shuffleId
                  val sizesOfMicroRdds = tracker.getStatisticsOfMicroRdds(shufDep)
                  sizesOfMicroRdds.foreach { case (microRddId: Int, sizes: mutable.Map[Long, Long])  =>
                    val prev: mutable.Map[Long, (Int, Long)] = sizesByMicroRddId.getOrElse(microRddId, mutable.Map[Long, (Int, Long)]())
                    sizesByMicroRddId.put(microRddId, prev ++ sizes.map { x =>
                      (x._1, (shufDep.shuffleId, x._2))
                    })
                  }
                }
                if (!mapStage.isAvailable || !mapStage.shuffleDep.shuffleMergeFinalized) {
              ...
              case narrowDep: NarrowDependency[_] =>
                rddToUpdate.prepend(rdd)
                waitingForVisit.prepend(narrowDep.rdd)
            }
          }
					if (shuffleIds.result().size > 0 && numShufDeps == shuffleIds.result().size) {
						MapOutputTracker.blocksByReduceId ++= assignBlocksToReduceId(shuffleIds.result().toArray.filter(shuffleId => !MapOutputTracker.blocksByReduceId.contains(shuffleId)), sizesByMicroRddId) 
						val shuffleId = shuffleIds.result()(0)
						val updatedNumPartitions = MapOutputTracker.blocksByReduceId(shuffleId).size
						if (rdd.id == stage.rdd.id) {
							stage.numPartitions = updatedNumPartitions
							stage match {
								case x: ResultStage =>
									globalWaiter.totalTasks = updatedNumPartitions
								case _ =>
							}
						}
						updatedPartitioner = new HashPartitioner(updatedNumPartitions)
						rdd.updatedPartitioner = updatedPartitioner
            //println("rdd.id: " + rdd.id + s"(${rdd.getClass})" + ", updated # of partitions: " + updatedNumPartitions)
						for (dep <- rdd.dependencies) {
							dep match {
								case shufDep: ShuffleDependency[_, _, _] =>
									val mapStage = getOrCreateShuffleMapStage(shufDep, stage.firstJobId)
									if (mapStage.isAvailable && mapStage.shuffleDep.shuffleMergeFinalized) {
										shufDep.rdd.updatedPartitioner = updatedPartitioner
                    //println("update rdd.id: " + shufDep.rdd.id + ", rdd.getClass: " + shufDep.rdd.getClass)
									}
							}
						}
						rdd match {
							case x: org.apache.spark.rdd.ShuffledRDD[_, _, _] =>
								x.part.asInstanceOf[HashPartitioner].partitions = updatedNumPartitions
							case x: org.apache.spark.rdd.CoGroupedRDD[_] =>
								x.part.asInstanceOf[HashPartitioner].partitions = updatedNumPartitions
							case _ =>
						}
					}
    ...
    if (updatedPartitioner != null) {
      rddToUpdate.foreach { rdd =>
				var updatedNumPartitions = 0
        var isComplete = true
				for (dep <- rdd.dependencies) {
					dep match {
						case narrowDep: NarrowDependency[_] =>
							val numPartitions = if (narrowDep.rdd.updatedPartitioner != null) {
								 narrowDep.rdd.updatedPartitioner.asInstanceOf[HashPartitioner].partitions
							} else {
                 isComplete = false
								 narrowDep.rdd.getNumPartitions
							}
							updatedNumPartitions += numPartitions
						case _ =>
					}
				}
        if (isComplete) {
        ...
        rdd match {
				  case x: org.apache.spark.rdd.UnionRDD[_] =>
            rdd.partitions
            rdd.updatedPartitioner = new HashPartitioner(updatedNumPartitions)
          case _ =>
        }
      }
    }
    //println("getMissingParentStages: " + ((System.currentTimeMillis - startTime) / 1000.0f))
    missing.toList
  }
  ...
  def submitJob[T, U](
    ...
    //println("maxPartitions in submitJob: " + maxPartitions)
  ...
  def runJob[T, U](
    ...
    globalWaiter = submitJob(rdd, func, partitions, callSite, resultHandler, properties)
    ThreadUtils.awaitReady(globalWaiter.completionFuture, Duration.Inf)
    globalWaiter.completionFuture.value.get match {
      case scala.util.Success(_) =>
        logInfo("Job %d finished: %s, took %f s".format
          (globalWaiter.jobId, callSite.shortForm, (System.nanoTime - start) / 1e9))
  ...
  private[scheduler] def handleJobSubmitted(jobId: Int,
    ...
    try {
      ...
      //println("# of partitions in handleJobSubmitted: " + partitions.size)
...
/*
  private def getSizesOfPartitions(stage: Stage): Array[Long] = {
    val rdd = stage.rdd
    if (rdd.dependencies.size == 1) {
      val dep = rdd.dependencies(0)
      dep match {
        case shufDep: ShuffleDependency[_, _, _] =>
          val mapStage = getOrCreateShuffleMapStage(shufDep, stage.firstJobId)
          if (mapStage.isAvailable && mapStage.shuffleDep.shuffleMergeFinalized) {
            import org.apache.spark._  // (MapOutputTrackerMaster 클래스 포함)
            val tracker = SparkEnv.get.mapOutputTracker.asInstanceOf[MapOutputTrackerMaster]
            val dep = mapStage.shuffleDep
            val sizesOfPartitions: Array[Long] = tracker.getStatisticsOfMapTasks(dep).bytesByPartitionId
            sizesOfPartitions
          } else {
            Array[Long]()
          }
  ...
  private def submitMissingTasks(stage: Stage, jobId: Int): Unit = {
    ...
    val numPartitions = if (stage.rdd.updatedPartitioner == null) {
      stage.rdd.partitions.length
    } else {
      stage.rdd.updatedPartitioner.asInstanceOf[HashPartitioner].partitions
    }

    var partitionsToCompute: Seq[Int] = stage.findMissingPartitions()
                                             .filter(id => id < numPartitions)
    ...
    stage match {
      ...
      case s: ResultStage =>
				globalWaiter.totalTasks = numPartitions
        outputCommitCoordinator.stageStart(
          stage = s.id, maxPartitionId = /*s.rdd.partitions.length*/numPartitions - 1)
        if (s.rdd.updatedPartitioner != null) {
          s.partitions = (0 until numPartitions/*s.rdd.partitions.length*/).toArray
        }
    }
    ...
    try {
      ...
      RDDCheckpointData.synchronized {
        ...
        //println("stage.rdd.partitions.length: " + stage.rdd.partitions.length)
        assert(stage.rdd.partitions.length == stage.rdd.getNumPartitions)
      }
    ...
    val tasks: Seq[Task[_]] = try {
      ...
      stage match {
        ...
        case stage: ResultStage =>
          partitionsToCompute.map { id =>
            val p: Int = id // stage.partitions(id)
