...
private[spark] class IndexShuffleBlockResolver(
   ...
   */
  def tmpWriteMetadataFileAndCommit(
      shuffleId: Int,
      mapId: Long,
      tmpLengths: Array[Long],
      tmpChecksums: Array[Long],
      dataTmp: File,
      lengthByMicroRddId: java.util.Map[java.lang.Integer, java.lang.Long] = null): Unit = {
    val startTime = System.currentTimeMillis
    var lengths = Array.fill[Long](lengthByMicroRddId.asScala.size)(0L)
    val checksums = if (lengths.size != tmpChecksums.size) {
      Array[Long]()
    } else {
      tmpChecksums
    }
    var kvs = Iterable[((Long, Int), Int)]()
    if (lengthByMicroRddId != null) {
      var cnt: Int = 0
      val partitionLengths = lengthByMicroRddId.asScala./*toIterable*/toList.sortBy(_._1).zipWithIndex
      //println("partitionLengths: " + partitionLengths)
      lengths = partitionLengths.map(x => x._1._2.toLong).toArray
      kvs = partitionLengths.map(x => ((mapId, x._1._1.toInt), x._2)).toMap
    } else {
      assert(false, "lengthByMicroRddId == null")
    }
    ...
    try {
      ...
      this.synchronized {
        if (kvs != null) {
          AdaptivePartitioner.idxByMicroRddId.getOrElseUpdate(shuffleId, scala.collection.mutable.Map[(Long, Int), Int]()) ++= kvs
        }
        ...
        if (existingLengths != null) {
          ...
          //println("lengths: " + lengths.toList)
  ...
  override def getMergedBlockData(
    ...
    assert(false, "getMergedBlockData")
    val indexFile =
  ...
  override def getBlockData(
    ...
    val (shuffleId, mapId, startReduceId, endReduceId) = blockId match {
      ...
      case batchId: ShuffleBlockBatchId =>
        assert(false, "ShuffleBlockBatchId")
    ...
    val idxByMicroRddId = AdaptivePartitioner.idxByMicroRddId(shuffleId)
    //println("idxByMicroRddId: " + idxByMicroRddId)
    //println("mapId: " + mapId + ", startReduceId: " + startReduceId.toInt + ", endReduceId: " + endReduceId.toInt)
    assert(idxByMicroRddId.contains((mapId, startReduceId.toInt)))
    val convertedStartReduceId = idxByMicroRddId.getOrElse((mapId, startReduceId.toInt), startReduceId)
    val convertedEndReduceId = idxByMicroRddId.getOrElse((mapId, endReduceId.toInt), convertedStartReduceId + 1)
    //println("convertedStartReduceId: " + convertedStartReduceId)
    //println("convertedEndReduceId: " + convertedEndReduceId)
    channel.position(convertedStartReduceId * 8L)
    val in = new DataInputStream(Channels.newInputStream(channel))
    try {
      val startOffset = in.readLong()
      channel.position(convertedEndReduceId/*endReduceId*/ * 8L)
      val endOffset = in.readLong()
      //println("startOffset: " + startOffset + ", endOffset: " + endOffset)
      val actualPosition = channel.position()
      val expectedPosition = convertedEndReduceId/*endReduceId*/ * 8L + 8
      if (actualPosition != expectedPosition) {
        assert(false, "actualPosition != expectedPosition")
