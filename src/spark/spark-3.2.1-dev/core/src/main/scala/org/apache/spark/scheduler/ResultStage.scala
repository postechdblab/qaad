...
private[spark] class ResultStage(
  ...
  var updatedPartitions = partitions // (modification)
  /**
  ...
  override def findMissingPartitions(): Seq[Int] = {
    ...
    if (rdd.updatedPartitioner != null) { // (modification)
      //println("job.numPartitions: " + job.numPartitions)
      job.finished = Array.fill[Boolean](job.numPartitions)(false)
    } else {
      //println("else job.numPartitions: " + job.numPartitions)
    }
