...
public class LocalDiskShuffleMapOutputWriter implements ShuffleMapOutputWriter {
  ...
  private java.util.HashMap<Integer, Long> lengthByMicroRddId;
  ...
  public LocalDiskShuffleMapOutputWriter(
    ...
    this.lengthByMicroRddId = new java.util.HashMap<Integer, Long>();
  ...
  public MapOutputCommitMessage commitAllPartitions(long[] checksums) throws IOException {
    ...
    blockResolver
      .tmpWriteMetadataFileAndCommit(shuffleId, mapId, partitionLengths, checksums, resolvedTmp, lengthByMicroRddId);
    return MapOutputCommitMessage.of(partitionLengths, lengthByMicroRddId);
  }
  ...
  private class PartitionWriterStream extends OutputStream {
    ...
    public void close() {
      isClosed = true;
      if (partitionId < partitionLengths.length)
        partitionLengths[partitionId] = count;
      lengthByMicroRddId.put(partitionId, count);
  ...
  private class PartitionWriterChannel implements WritableByteChannelWrapper {
    ...
    public void close() throws IOException {
      ...
      lengthByMicroRddId.put(partitionId, getCount());
      //bytesWrittenToMergedFile += partitionLengths[partitionId];
      bytesWrittenToMergedFile += lengthByMicroRddId.get(partitionId);
    }
  }
}
