...
import org.apache.spark.mllib.fpm.{MyAssociationRules => MyMLlibAssociationRules, MyFPGrowth => MyMLlibFPGrowth}
import org.apache.spark.mllib.fpm.FPGrowth.FreqItemset
import org.apache.spark.mllib.fpm.MyFPGrowth.MyFreqItemset
...
private[fpm] trait MyFPGrowthParams extends Params with HasPredictionCol {
  ...
  val maxLength: IntParam = new IntParam(this, "maxLength", "maximal length for generating Associationo Rule", ParamValidators.gtEq[Int](-1)) // maxLength XXX
  setDefault(maxLength -> -1)

  def getMaxLength: Int = $(maxLength)

  val minLift: DoubleParam = new DoubleParam(this, "minLift", "minimal lift for generating Associationo Rule", ParamValidators.gtEq[Double](-1.0))
  setDefault(minLift -> -1)

  def getMinLift: Double = $(minLift)

  val minChiSquare: DoubleParam = new DoubleParam(this, "minChiSquare", "minimal chi-square for generating Associationo Rule", ParamValidators.gtEq[Double](-1.0))
  setDefault(minChiSquare -> -1)

  def getMinChiSquare: Double = $(minChiSquare)

  val minKulc: DoubleParam = new DoubleParam(this, "minKulc", "minimal kulc for generating Associationo Rule", ParamValidators.gtEq[Double](-1.0))
  setDefault(minKulc -> -1)

  def getMinKulc: Double = $(minKulc)

  val minImbalanceRatio: DoubleParam = new DoubleParam(this, "minImbalanceRatio", "minimal imbalance ratio for generating Associationo Rule", ParamValidators.gtEq[Double](-1.0))
  setDefault(minImbalanceRatio -> -1)

  def getMinImbalanceRatio: Double = $(minImbalanceRatio)

  //val count: LongParam = new LongParam(this, "count", "count for generating Associationo Rule", ParamValidators.gtEq[Long](-1))
...
class MyFPGrowth @Since("2.2.0") (
    @Since("2.2.0") override val uid: String)
  extends Estimator[MyFPGrowthModel] with MyFPGrowthParams with DefaultParamsWritable {

  @Since("2.2.0")
  def this() = this(Identifiable.randomUID("myfpgrowth"))
  ...
  def setMinChiSquare(value: Double): this.type = set(minChiSquare, value)

  def setMinKulc(value: Double): this.type = set(minKulc, value)

  def setMinImbalanceRatio(value: Double): this.type = set(minImbalanceRatio, value)
  ...
  @Since("2.2.0")
  override def fit(dataset: Dataset[_]): MyFPGrowthModel = {
    transformSchema(dataset.schema, logging = true)
    genericFit(dataset)
  }

  private def genericFit[T: ClassTag](dataset: Dataset[_]): MyFPGrowthModel = instrumented { instr =>
    val handlePersistence = dataset.storageLevel == StorageLevel.NONE
    ...
    val data = dataset.select($(itemsCol))
    val items = data.where(col($(itemsCol)).isNotNull).rdd.map(r => r.getSeq[Any](0).toArray)
    val mllibFP = new MyMLlibFPGrowth().setMinSupport($(minSupport))
    if (isSet(numPartitions)) {
      mllibFP.setNumPartitions($(numPartitions))
    }
    if (isSet(maxLength)) {
      mllibFP.setMaxLength($(maxLength))
    }
    if (isSet(minLift)) {
      mllibFP.setMinLift($(minLift))
    }
    if (isSet(minChiSquare)) {
      mllibFP.setMinChiSquare($(minChiSquare))
    }
    if (isSet(minKulc)) {
      mllibFP.setMinKulc($(minKulc))
    }
    if (isSet(minImbalanceRatio)) {
      mllibFP.setMinImbalanceRatio($(minImbalanceRatio))
    }
    mllibFP.setCount(count)
    ...
    copyValues(new MyFPGrowthModel(uid, frequentItems, parentModel.itemSupport, inputRowCount, count))
      .setParent(this)
  }
  ...
  @Since("2.2.0")
  override def copy(extra: ParamMap): MyFPGrowth = defaultCopy(extra)
}
...
object MyFPGrowth extends DefaultParamsReadable[MyFPGrowth] {

  @Since("2.2.0")
  override def load(path: String): MyFPGrowth = super.load(path)
}
...
class MyFPGrowthModel private[ml] (
    @Since("2.2.0") override val uid: String,
    @Since("2.2.0") @transient val freqItemsets: DataFrame,
    private val itemSupport: scala.collection.Map[Any, Double],
    private val numTrainingRecords: Long,
    private val count: Long)
  extends Model[MyFPGrowthModel] with MyFPGrowthParams with MLWritable {
   ...
   */
  @transient private var _cachedMinConf: Double = Double.NaN
  @transient private var _cachedMinLift: Double = Double.NaN
  @transient private var _cachedMinChiSquare: Double = Double.NaN
  @transient private var _cachedMinKulc: Double = Double.NaN
  @transient private var _cachedMinImbalanceRatio: Double = Double.NaN
  //@transient private var _cachedCount: Long = 0
  ...
  @transient def associationRules: DataFrame = {
    if ($(minConfidence) == _cachedMinConf) {
      ...
      _cachedRules = MyAssociationRules
        .getAssociationRulesFromFP(freqItemsets, "items", "freq", $(minConfidence), $(minLift), $(minChiSquare), $(minKulc), $(minImbalanceRatio), count, itemSupport)
      _cachedMinConf = $(minConfidence)
      _cachedMinLift = $(minLift)
      _cachedMinChiSquare = $(minChiSquare)
      _cachedMinKulc = $(minKulc)
      _cachedMinImbalanceRatio = $(minImbalanceRatio)
      //_cachedCount = $(count)
      _cachedRules
    }
  }
  ...
  @Since("2.2.0")
  override def copy(extra: ParamMap): MyFPGrowthModel = {
    val copied = new MyFPGrowthModel(uid, freqItemsets, itemSupport, numTrainingRecords, count)
    copyValues(copied, extra).setParent(this.parent)
  }

  @Since("2.2.0")
  override def write: MLWriter = new MyFPGrowthModel.MyFPGrowthModelWriter(this)

  @Since("3.0.0")
  override def toString: String = {
    s"MyFPGrowthModel: uid=$uid, numTrainingRecords=$numTrainingRecords"
  }
}
...
object MyFPGrowthModel extends MLReadable[MyFPGrowthModel] {

  @Since("2.2.0")
  override def read: MLReader[MyFPGrowthModel] = new MyFPGrowthModelReader

  @Since("2.2.0")
  override def load(path: String): MyFPGrowthModel = super.load(path)

  /** [[MLWriter]] instance for [[MyFPGrowthModel]] */
  private[MyFPGrowthModel]
  class MyFPGrowthModelWriter(instance: MyFPGrowthModel) extends MLWriter {
  ...
  private class MyFPGrowthModelReader extends MLReader[MyFPGrowthModel] {

    /** Checked against metadata when loading model */
    private val className = classOf[MyFPGrowthModel].getName

    override def load(path: String): MyFPGrowthModel = {
      implicit val format = DefaultFormats
      val metadata = DefaultParamsReader.loadMetadata(path, sc, className)
      val (major, minor) = VersionUtils.majorMinorVersion(metadata.sparkVersion)
      val numTrainingRecords = if (major.toInt < 2 || (major.toInt == 2 && minor.toInt < 4)) {
      ...
      val model = new MyFPGrowthModel(metadata.uid, frequentItems, itemSupport, numTrainingRecords, 0)
      metadata.getAndSetParams(model)
      model
    }
  }
...
private[fpm] object MyAssociationRules {
  ...
  def getAssociationRulesFromFP[T: ClassTag](
        ...
        minChiSquare: Double,
        minKulc: Double,
        minImbalanceRatio: Double,
        count: Long,
        itemSupport: scala.collection.Map[T, Double]): DataFrame = {

    val freqItemSetRdd = dataset.select(itemsCol, freqCol).rdd
      .map(row => new MyFreqItemset(row.getSeq[T](0).toArray, row.getLong(1)))
    val dt = dataset.schema(itemsCol).dataType
    val add_metric = minLift >= 0.0 && minChiSquare >= 0.0 && minKulc >= 0.0 && minImbalanceRatio >= 0.0
    if (add_metric) {
      val rows = new MyMLlibAssociationRules()
      .setMinConfidence(minConfidence)
      .setMinLift(minLift)
      .setMinChiSquare(minChiSquare)
      .setMinKulc(minKulc)
      .setMinImbalanceRatio(minImbalanceRatio)
      .setCount(count)
      .run(freqItemSetRdd, itemSupport, minLift, minChiSquare, minKulc, minImbalanceRatio, count)
      .map(r => Row(r.antecedent, r.consequent, r.order, r.support, r.confidence, r.lift.orNull, r.chiSquare.orNull, r.kulc.orNull, r.imbalanceRatio.orNull))
      val schema = StructType(Seq(
        StructField("antecedent", dt, nullable = false),
        StructField("consequent", dt, nullable = false),
        ...
        StructField("confidence", DoubleType, nullable = false),
        StructField("lift", DoubleType),
        StructField("chiSquare", DoubleType),
        StructField("kulc", DoubleType),
        StructField("imbalanceRatio", DoubleType)))
      val rules = dataset.sparkSession.createDataFrame(rows, schema)
      rules
    } else {
      val rows = new MyMLlibAssociationRules()
