...
__all__ = ['FPGrowth', 'MyFPGrowth', 'FPGrowthModel', 'MyFPGrowthModel', 'PrefixSpan', 'PrefixSpanModel', 'CSpade', 'CSpadeModel']
...
@inherit_doc
@ignore_unicode_prefix
class MyFPGrowthModel(JavaModelWrapper, JavaSaveable, JavaLoader):
    """
    A FP-Growth model for mining frequent itemsets
    using the Parallel FP-Growth algorithm.
    ...
    def freqItemsets(self):
        ...
        Returns the frequent itemsets of this model.
        """
        return self.call("getFreqItemsets").map(lambda x: (MyFPGrowth.FreqItemset(x[0], x[1])))
    ...
    def load(cls, sc, path):
        ...
        """
        model = cls._load_java(sc, path)
        wrapper = sc._jvm.org.apache.spark.mllib.api.python.MyFPGrowthModelWrapper(model)
        return MyFPGrowthModel(wrapper)
...
class MyFPGrowth(object):
    ...
    @classmethod
    @since("1.4.0")
    def train(cls, data, minSupport=0.3, maxLength=-1, minLift=-1.0, minChiSquare=-1.0, minKulc=-1.0, minImbalanceRatio=-1.0, numPartitions=-1): # maxLength XXX
        """
        Computes an FP-Growth model that contains frequent itemsets.
        ...
        :param numPartitions:
          ...
          (default: -1)
        """
        model = callMLlibFunc("trainMyFPGrowthModel", data, float(minSupport), maxLength, minLift, minChiSquare, minKulc, minImbalanceRatio, int(numPartitions)) # maxLength XXX
        return MyFPGrowthModel(model)
...
@inherit_doc
@ignore_unicode_prefix
class CSpadeModel(JavaModelWrapper):
    """
    Model fitted by CSpade
    ...
    def freqSequences(self):
        """Gets frequent sequences"""
        return self.call("getFreqSequences").map(lambda x: CSpade.FreqSequence(x[0], x[1], x[2]))
...
class CSpade(object):
    ...
    @classmethod
    @since("1.6.0")
    def train(cls, data, minSupport=0.1, maxPatternLength=10, maxWidth=10, maxGap=-1, minGap=0, window=100, minConfidence=0.7, minLift=0.0, removeRepetitivePattern=False):
        """
        Finds the complete set of frequent sequential patterns in the
        input sequences of itemsets.
        ...
        :param maxPatternLength:
          ...
          (default: 10)
        """
        model = callMLlibFunc("trainCSpadeModel",
                              data, minSupport, maxPatternLength, maxWidth, maxGap, minGap, window, minConfidence, minLift, removeRepetitivePattern)
        return CSpadeModel(model)

    class FreqSequence(namedtuple("FreqSequence", ["sequence", "freq", "sids"])):
        """
        Represents a (sequence, freq) tuple.
