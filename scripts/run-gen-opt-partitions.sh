...
if [ ${dataset} == bra ]; then
  /root/dev/hadoop-2.7.7/bin/hdfs dfs -rm -r -f /root/QaaD/datasets/synthetic-brazilian-ecommerce/num-rows-${num_rows}/*
  /root/dev/hadoop-2.7.7/bin/hdfs dfs -mkdir -p /root/QaaD/datasets/synthetic-brazilian-ecommerce/num-rows-${num_rows}/
  /root/dev/spark-3.2.1-bin-hadoop2.7/bin/spark-shell \
    ...
    -i /root/QaaD/src/SetupForGenOptDataset.scala \
    -i /root/QaaD/querysets/brazilian-ecommerce/load.scala \
    -i /root/QaaD/src/GenDataset-bra.scala > /dev/null 2>&1
else
  /root/dev/hadoop-2.7.7/bin/hdfs dfs -rm -r -f /root/QaaD/datasets/synthetic-ebay/num-rows-${num_rows}/*
  /root/dev/hadoop-2.7.7/bin/hdfs dfs -mkdir -p /root/QaaD/datasets/synthetic-ebay/num-rows-${num_rows}/
  /root/dev/spark-3.2.1-bin-hadoop2.7/bin/spark-shell \
    --master yarn \
    --driver-memory ${m}g \
    --driver-cores 14 \
