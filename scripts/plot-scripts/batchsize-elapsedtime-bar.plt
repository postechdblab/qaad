...
set xlabel "Batch size" offset 0,1.2
...
plot \
  ...
  _sparksdata using (log($1)/log(2)-0.3):3 notitle with boxes lc rgb "#bfefff", \
  sparkudata using (log($1)/log(2)):3:xtic(2) title "SparkU" with boxes lc rgb "#808080", \
  _sparkudata using (log($1)/log(2)):3:xtic(2) notitle with boxes lc rgb "#669acc", \
  qaaddata using (log($1)/log(2)+0.3):3 title "QaaD" with boxes lc rgb "#000000", \
  _qaaddata using (log($1)/log(2)+0.3):3 notitle with boxes lc rgb "#003366"
